...
function App() {
  const [user, setUser] = useState({
    ...
    const handdleAddressChange = (event : React.ChangeEvent<HTMLInputElement>) => {
      console.log(event.target.value);

      // const address =
      // user.extra.addressBook.find(address => address.id === Number(event.target.id));
      ...
      const newAddressBook = user.extra.addressBook.map(address => {
        if(address.id === Number(event.target.id)){ //바꿔야하는 주소
      ...
      console.log('회사', user.extra.addressBook[0].value === newUser.extra.addressBook[0].value);
      console.log('집', user.extra.addressBook[1] === newUser.extra.addressBook[1]);
      console.log('집', user.extra.addressBook[1].value === newUser.extra.addressBook[1].value);
      setUser(newUser)
  ...
  return (
    <>
      ...
      <p>
        <EditAddress addressBook = { user.extra.addressBook } handdleAddressChange = {handdleAddressChange}/>
      </p>
    </>
  )
}
